...
<body>
    <div class="contain">
        <div id="wrapper">
            <form method="POST" action="{{route('user_login_haha')}}" id="form-signin">
                @csrf
                <h1 class="form-heading">Đăng Nhập</h1>
                <div class="form-group">
                    <i class="far fa-envelope"></i>
                    <input id="email" type="text" class="form-input" placeholder="Email" name="email" required>
                </div>
                <div class="form-group">
                    <i class="fas fa-key"></i>
                    <input id="ipnPassword" type="password" class="form-input" placeholder="Mật khẩu" name="password" required>
                    <div id="eye">
                        <i id="btnPassword" class="far fa-eye"></i>
                    </div>
                </div>  
                    <p class="form_link"><a href="{{url('user/register')}}">Bạn muốn đăng ký?</a></p>
                    <p class="form_link"><a href="{{ route('password.request') }}">Quên mật khẩu?</a></p>
                <button type="submit" class="form-submit">{{ __('Log in') }}</button>
            </form>
        </div>
    </div>
</body>
